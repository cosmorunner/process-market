                    ...
                    <div class="d-flex justify-content-between">
                        <div>
                            <span class="badge badge-light" v-if="field.items.length">{{
                                    field.items.length
                                }} {{ field.items.length === 1 ? 'Option' : 'Optionen' }}</span>
                            ...
                            <span class="badge badge-light" v-if="field.as_string">Wert als Zeichenkette</span>
                        </div>
                        <div></div>
                    </div>
                </div>
...
export default {
    ...
    computed: {
        ...
        shouldDisplayHr() {
            return (this.field.label || this.field.helper_text) && (this.field.css_classes || (this.field.mapping && Object.keys(this.field.mapping).length) || this.field.items.length || (this.field.list_config && this.field.list_config.id) || this.field.highlighted || this.field.as_string);
        }
    },
};
</script>
