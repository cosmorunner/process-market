    ...
    <div class="card-header striped-bg p-2 d-flex justify-content-between border-bottom">
        ...
        <div class="d-flex justify-content-end">
            <a class="d-inline-block px-2"
               href="{{route('license.demo', ['license' => $license, 'version' => 'latest']) . '?ref=' . base64_encode(route('organisation.licenses', $license->owner))}}">Demo</a>
            @if($license->allowsCopy() && $role?->can('processes.create'))
                <a class="d-inline-block px-2"
                   href="{{route('process.create', ['template' => $license->resource->full_namespace, 'namespace' => $organisation->namespace])}}">Als
                    Vorlage...</a>
            @endif
            @if($role?->can('process_versions.export'))
                <a class="d-inline-block px-2"
                   href="{{route('license.sync', ['license' => $license, 'version' => 'latest'])}}">Export</a>
            @endif
        </div>
    </div>
    @if($license->resource->description || count($license->resource->tags))
        <div class="card-body p-2">
            @if($license->resource->description)
                <p class="card-text mb-2">{{ Str::limit((string) $license->resource->description, 250) }}</p>
            @endif
    ...
    <div class="card-footer d-flex justify-content-between bg-white p-2">
        <small class="text-muted d-block text-truncate align-content-center d-flex">
            <a href="{{$license->resource->author->publicPath()}}">
                <img src="{{$license->resource->author->thumbnailPath()}}" class="d-inline-block rounded-circle mr-1"
                     height="25" alt=""/>
            </a>
            <span>
