...
@section('content')
    <div class="container mt-4">
        <div class="row mb-3">
            <div class="col">
                <a href="{{route('process.versions', ['process' => $processVersion->process, 'limit' => 10])}}">Zu den Versionen</a>
            </div>
        </div>
        <div class="row mb-3">
            <div class="col">
                    ...
                    <div class="container bg-white border p-3">
                        <div class="row">
                            <div class="col">
                                ...
                                <form method="POST" action="{{ route('process_version.execute_sync', ['processVersion' => $processVersion]) }}">
                                    @csrf
                                                ...
                                                @if($lastSynchronization = $system->synchronizationsOf($processVersion)->first())
                                                    ...
                                                    @else
                                                        <small class="text-danger">Fehler
                                                            am {{\Carbon\Carbon::createFromTimeString($lastSynchronization->created_at)->format('d.m.Y')}}: {{$lastSynchronization->response_message}}</small>
                                                    @endif
                                                @endif
                                            </li>
                                        @endforeach
