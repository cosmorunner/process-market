<div>
    <a data-toggle="collapse" href="#dependencies-{{$processVersion->id}}"
       aria-expanded="false" aria-controls="dependencies-{{$processVersion->id}}">
    ...
    <div class="collapse mt-2" id="dependencies-{{$processVersion->id}}">
                ...
                <ul class="list-group border-0">
                    @foreach($dependencies['connector_request_identifiers'] ?? [] as $connectorIdentifier => $requestIdentifiers)
                        @foreach($requestIdentifiers as $requestIdentifier)
                            <li class="border-0 text-muted list-group-item p-0 pl-3">{{$connectorIdentifier}}/{{$requestIdentifier}}</li>
                        @endforeach
                    @endforeach
                </ul>
            </div>
        ...
        @if($dependencies['variable_identifiers'] ?? null)
            <div class="mb-2">
                ...
                <ul class="list-group border-0">
                    @php
                        $identifiers = $dependencies['variable_identifiers'] ?? [];
                        sort($identifiers);
                    @endphp
                    @foreach($identifiers as $dependency)
                        <li class="border-0 text-muted list-group-item p-0 pl-3">{{$dependency}}</li>
                    @endforeach
                </ul>
            </div>
        @endif
        @if($dependencies['task_identifiers'] ?? null)
            <div class="mb-2">
                <span>Aufgaben</span>
                <ul class="list-group border-0">
                    @php
                        $identifiers = $dependencies['task_identifiers'] ?? [];
                        sort($identifiers);
                    @endphp
                    @foreach($identifiers as $dependency)
                        <li class="border-0 text-muted list-group-item p-0 pl-3">{{$dependency}}</li>
