    ...
    <div class="card-header p-2 d-flex justify-content-between border-bottom">
        ...
        <div class="d-flex justify-content-end">
            ...
            <a class="d-inline-block px-2"
               href="{{route('process.demo', ['process' => $process, 'version' => 'develop']) . '?ref=' . base64_encode('profile.processes')}}">Demo</a>
            @if($archived)
            ...
            <div class="dropdown d-inline-block ml-2">
                <button class="btn btn-light py-0 btn-sm" type="button" id="dropdownMenuButton2" data-toggle="dropdown"
                ...
                <div class="dropdown-menu scrollable-dropdown" aria-labelledby="dropdownMenuButton2">
                    ...
                    @if(!$archived)
                        @if($process->hasPublishedVersion())
                            <a class="dropdown-item"
                               href="{{route('process.create', ['template' => $process->full_namespace, 'namespace' => $user->namespace])}}">Als
                                Vorlage...</a>
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item text-danger" href="{{route('process.delete', $process)}}">Archivieren</a>
                        @else
                            <a class="dropdown-item text-danger"
                               href="{{route('process.delete', $process)}}">Löschen</a>
                        @endif
                    @endif
                </div>
            </div>
