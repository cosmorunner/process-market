    ...
    <div class="input-group input-group-sm mb-2 ">
        <div class="input-group-append">
            ...
            <div class="dropdown-menu scrollable-dropdown" aria-labelledby="concatDropdown">
                <button type="button" class="dropdown-item" @click="onChangeConcatSplit(null)">
                    Deaktiviert
                </button>
        ...
        <div class="input-group-append" v-if="concat.length > 1">
            <button
                :class="'btn btn-sm dropdown-toggle btn-outline-' + (selectableAliases.includes(concat[1]) ? 'success' : 'danger')"
        ...
        <div class="input-group-append" v-if="concat.length > 1">
            <button
                :class="'btn btn-sm dropdown-toggle btn-outline-' + (selectableAliases.includes(concat[2]) ? 'success' : 'danger')"
            ...
            <div class="dropdown-menu scrollable-dropdown" aria-labelledby="secondConcatDropdown">
                <button type="button" class="dropdown-item" @click="onChangeConcatField(2, '')">Ohne Wert</button>
                <div class="dropdown-divider"></div>
                <button type="button" v-for="alias in selectableAliases" class="dropdown-item"
                        @click="onChangeConcatField(2, alias)">{{ aliasLabels[alias] || alias }}
                </button>
            </div>
        </div>
    </div>
