 ...
 */
class StoreUser implements ValidationRule {
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void {
        ...
        $existingUserAliases = $this->environment->blueprint->users->pluck('aliases')->flatten()->unique();
        $existingUserEmails = $this->environment->blueprint->users->pluck('email')->flatten()->unique();
        $existingProcessInstances = $this->environment->blueprint->processes->pluck('id')->flatten()->unique();

        $validator = Validator::make((array) $value, [
            'id' => ['required', 'uuid', Rule::notIn($this->environment->blueprint->users->pluck('id'))],
